...
def add_peer_netd_start_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--node-id", default="amo-node", help="Stable internal AMO node id advertised by the sidecar.")
...
def add_peer_subcommands(sub: Any) -> None:
    ...
    peer_public_invite = peer_sub.add_parser("invite", help="Create a human-shareable AMO peer invite")
    peer_public_invite.add_argument("--trust", choices=["trusted", "limited", "blocked"], default="trusted")
    peer_public_invite.add_argument("--label", default="", help="Optional human-readable invite label.")
    peer_public_invite.add_argument("--relay-profile", "--relay", dest="relay_profile", default="", help="Saved relay profile to include in this invite.")
    peer_public_invite.add_argument("--expires-minutes", type=int, default=1440, help="Invite validity window.")
    peer_public_invite.add_argument("--max-uses", type=int, default=1, help="Maximum accepted join requests.")
    peer_public_invite.add_argument("--manual-approval", action="store_true", help="Require manual approval for the friend's join request.")
    peer_public_invite.add_argument("--json", action="store_true", help="Print machine-readable invite details.")
    ...
    peer_setup.add_argument("--install-startup", action="store_true", help="Compatibility flag; startup/watch is installed by default.")
    peer_setup.add_argument("--no-startup", action="store_true", help="Do not install OS startup entries for sidecar and peer-agent watch.")
    ...
    peer_setup.add_argument("--peer-netd-manifest", default="", help="Signed peer-netd artifact manifest URL/path.")
    ...
    peer_join = peer_sub.add_parser("join", help="Join a trusted AMO peer from an invite code")
    add_peer_netd_start_args(peer_join)
    peer_join.add_argument("--display-name", default="", help="Display name to save for this AMO peer identity.")
    peer_join.add_argument("--invite-code", default="", help="amo-peer-invite: code to accept.")
    peer_join.add_argument("--trust", choices=["trusted", "limited", "blocked"], default="")
    peer_join.add_argument("--service-name", default="AMO Peer Netd")
    add_peer_netd_watch_service_args(peer_join)
    peer_join.add_argument("--no-startup", action="store_true", help="Do not install OS startup entries for sidecar and peer-agent watch.")
    peer_join.add_argument("--no-start", action="store_true", help="Only save config/profile; do not start peer-netd now.")
    peer_join.add_argument("--peer-netd-manifest", default="", help="Signed peer-netd artifact manifest URL/path.")
    peer_join.add_argument("--peer-netd-version", default="latest", help="peer-netd artifact version to install.")
    peer_join.add_argument("--skip-sidecar-install", action="store_true", help=argparse.SUPPRESS)
    peer_join.add_argument("--yes", action="store_true", help="Use defaults for non-interactive join prompts.")
    peer_join.add_argument("--json", action="store_true", help="Print machine-readable join details.")
    ...
    peer_agent_discuss = peer_agent_sub.add_parser("discuss", help="Run an automated bounded peer-agent discussion lifecycle")
    peer_agent_discuss.add_argument("--query", required=True)
    peer_agent_discuss.add_argument("--peer", action="append", default=[], help="Trusted peer node id to ask. Repeat for multiple peers.")
    peer_agent_discuss.add_argument("--session-id", default="")
    peer_agent_discuss.add_argument("--min-confidence", type=float, default=None)
    peer_agent_discuss.add_argument("--timeout-seconds", type=float, default=None, help="Per-turn peer response timeout.")
    peer_agent_discuss.add_argument("--max-turns", type=int, default=4)
