...
def test_architecture_source_roots_have_explicit_product_ownership() -> None:
    ...
    optional_generated_roots = {"bin"}
    actual_roots = {
        path.name
        for path in src_root.iterdir()
        if path.is_dir() and path.name not in {"__pycache__", *optional_generated_roots}
    }
    product_domain_roots = {"domain", "evidence", "versioning", "peer"}
    product_application_roots = {"application"}
    infrastructure_roots = {"infrastructure", "llm", "integrations", "install"}
    ...
    for root in optional_generated_roots:
        assert f"`src/agent_memory_orchestrator/{root}/` is optional" in architecture_tree
